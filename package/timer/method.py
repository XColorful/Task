...
from .function import YYYY_MM_DD, input_is_YYYY_MM_DD, convert_to_int, show_task_info, check_timer_u, check_timer_t, check_timer_all
...
def get_match_func_timer_index_list(tasker_list, timer_tasker_index, check_func) -> list[list]:
    """返回列表格式：
    ...
    for tasker_index in timer_tasker_index:
        ...
        # 检查timer_task是否匹配
        for task_index, timer_task in enumerate(tasker_list[tasker_index]):
            if check_func(timer_task):
...
class timer_task_method(default_method_template):
    ...
    def timer(self, cmd_parameter:str, tasker_list:list, system_pkg:dict) -> tuple:
        ...
        date_YYYY_MM_DD = select_YYYY_MM_DD(date_parameter)
        ...
        matched_index_list = get_match_func_timer_index_list(tasker_list, timer_tasker_index, check_func)
