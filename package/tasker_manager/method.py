    ...
    def add_func(self, cmd_parameter:str, tasker_list:list, system_pkg:dict):
        ...
        if tasker_type == system_pkg["TYPE_DEFAULT_TASKER"]:
            ...
            if class_func_type == system_pkg["TYPE_EXTRA_CLASS_FUNC"]:
                ...
                return (system_pkg["CONDITION_SUCCESS"], f"\"{class_func.label}\"（extra类型）不支持容器\"{tasker.tasker_label}\"")
            # class_func为default类型
                ...
                if class_func_version == None:
                    ...
                    if class_func_version > convert_to_float(tasker_version):
                        ...
                        return (system_pkg["CONDITION_SUCCESS"], f"\"{class_func.label}\"版本不支持容器\"{tasker.tasker_label}\"")
        # tasker为extra类型
        elif tasker_type == system_pkg["TYPE_EXTRA_TASKER"]:
            # class_func为extra类型
            if class_func_type == system_pkg["TYPE_EXTRA_CLASS_FUNC"]:
                if class_func_version != tasker_version: # extra版本不同
                    system_pkg["system_msg"](f"容器\"{tasker_type}\"（extra类型）仅支持相同版本的class_func（extra类型）")
                    return (system_pkg["CONDITION_SUCCESS"], f"\"{class_func.label}\"（extra类型）不支持容器\"{tasker.tasker_label}\"（extra类型）")
            # class_func为default类型
            else: # 进行默认版本比较
                suit_task = False
                error_task = False
                ...
                if error_task == True:
                    system_pkg["system_msg"](f"容器\"{tasker.tasker_label}\"（extra类型）内包含低版本task类型模板")
                    return (system_pkg["CONDITION_SUCCESS"], f"\"{class_func.label}\"（extra类型）不支持容器\"{tasker.tasker_label}\"（extra类型）（含有低版本task模板）")
                if suit_task == False:
                    system_pkg["system_msg"](f"容器\"{tasker.tasker_label}\"（extra类型）内无可用的默认task类型模板")
                    return (system_pkg["CONDITION_SUCCESS"], f"\"{class_func.label}\"（extra类型）不支持容器\"{tasker.tasker_label}\"（extra类型）（无可用的默认task模板）")
        # tasker为未知类型
        else:
            system_pkg["system_msg"]("容器类型错误")
            return (system_pkg["CONDITION_FAIL"], f"容器{tasker.tasker_label}类型（{tasker_type}）异常")
        # 确认是否添加
